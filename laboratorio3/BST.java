...
import java.util.*;
    ...
    public void put(Key key, Value val) {
        ...
        assert check();//comprueba que se cumplan las condiciones del bst
    }
     ...
     */
                            ...
                            //entrega llave entre lo y hi, aqui necesitamos una cola
  ...
  public Iterable<Value> values(Key lo, Key hi) {
        ...
        values(root, arr, lo, hi);
        return arr;
        
    } 

    private void values(Node x, ArrayList<Value> arr, Key lo, Key hi) { 
        if (x == null) return; 
        int cmplo = lo.compareTo(x.key); 
        int cmphi = hi.compareTo(x.key); 
        if (cmplo < 0) values(x.left, arr, lo, hi); 
        if (cmplo <= 0 && cmphi >= 0) arr.add(x.val); 
        if (cmphi > 0) values(x.right, arr, lo, hi); 
    } 
    ...
    //cuantas LLAVES hay entre lo y hi
